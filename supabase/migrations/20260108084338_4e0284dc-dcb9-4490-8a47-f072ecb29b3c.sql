-- Insert comprehensive evaluation scale points for all exam systems
-- This data will be available system-wide

-- First, let's create a function to ensure symbol points exist for an organization
CREATE OR REPLACE FUNCTION public.seed_symbol_points_for_organization(org_id UUID)
RETURNS void AS $$
BEGIN
  -- Only insert if no symbol points exist for this organization
  IF NOT EXISTS (SELECT 1 FROM public.symbol_points WHERE organization_id = org_id) THEN
    ...
    -- NSSC AS (Namibia Senior Secondary Certificate Advanced Subsidiary)
    INSERT INTO public.symbol_points (organization_id, exam_level, symbol, points, active) VALUES
...
-- Seed symbol points for all existing organizations
DO $$
DECLARE
  org RECORD;
BEGIN
  FOR org IN SELECT id FROM public.organizations LOOP
    PERFORM public.seed_symbol_points_for_organization(org.id);
  END LOOP;
END $$;

-- Create trigger to auto-seed symbol points for new organizations
CREATE OR REPLACE FUNCTION public.trigger_seed_symbol_points()
RETURNS TRIGGER AS $$
BEGIN
  PERFORM public.seed_symbol_points_for_organization(NEW.id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

DROP TRIGGER IF EXISTS seed_symbol_points_on_org_create ON public.organizations;
CREATE TRIGGER seed_symbol_points_on_org_create
  AFTER INSERT ON public.organizations
  FOR EACH ROW
  EXECUTE FUNCTION public.trigger_seed_symbol_points();