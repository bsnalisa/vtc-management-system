 ...
 Deno.serve(async (req) => {
   ...
   try {
     ...
     // If fully cleared, update registration and trainee status
     if (isFullyCleared) {
       const traineeId = queueEntry.entity_id
 
       // Get trainee to find their registration
       ...
       if (trainee) {
         ...
         await supabaseAdmin
           ...
           .eq('registration_status', 'fee_pending')
         ...
         // Update application hostel status if applicable
         const { data: registration } = await supabaseAdmin
           .from('registrations')
           .select('hostel_required, application_id')
           .eq('trainee_id', traineeId)
           .single()
 
         if (registration?.hostel_required && registration.application_id) {
           await supabaseAdmin
             .from('trainee_applications')
             .update({ hostel_application_status: 'allocated' })
       ...
       console.log(`Registration fee cleared for trainee ${traineeId}`)
     }
